...
const QuranVerseClient: React.FC<QuranVerseProps> = ({ verses, surahNumber, surahName, initialLanguage }) => {
  ...
  if (!verses || verses.length === 0) {
    return <div>No verses found for this surah</div>;
  }
...
<div className="
relative top-0 right-0 mr-6 mt-6 text-white hover:bg-gray-500 rounded-lg w-12 h-12 flex items-center justify-center p-2">
        
  <svg onClick={toggleDropdown} className="cursor-pointer" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {showDropdown && (
  <div className="relative top-full right-0 mt-2 bg-gray-800 text-white rounded-md shadow-lg max-w-xs"> {/* Added max-w-xs */}
...
>
  <FaBookmark className={bookmarkedVerses.includes(verse.id) ? 'text-yellow-500 hover:text-yellow-700 transition-colors duration-300' : ''} />
              ...
              <p className="english-verse-text">
                {selectedLanguage === 'en'
                  ? verse.englishTranslation?.[0]?.text.replace(/<sup.*?<\/sup>/g, '').trim() || 'Translation not available'
                  : selectedLanguage === 'ur'
                  ? verse.urduTranslation?.[0]?.text.replace(/<sup.*?<\/sup>/g, '').trim() || 'Translation not available'
                  : verse.hindiTranslation?.[0]?.text.replace(/<sup.*?<\/sup>/g, '').trim() || 'Translation not available'}
              </p>
            </div>
            </div>
            {index < verses.length + 1 && (
              <div className="flex flex-col items-start">
                <hr className="surah-separator w-full" />
              </div>
            )}
          </React.Fragment>
        ))}
...
export default QuranVerseClient;
